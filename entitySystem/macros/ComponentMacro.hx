	...
	macro public static function build():Array<Field> {
		...
		fields.push({
			...
			kind: FFun({
				...
				params: [],
				args: [],
				expr: mk(EReturn(mk(EConst(CString(typeName)))))
			}),
			pos: pos
		});
		#end
		...
		fields.push({
			...
			meta: [],
			access: [APublic],
			kind: FVar(tint, {expr: EConst(CInt(Std.string("0"))), pos: pos}),
			pos: pos
		});
		fields.push({
			name: "nextProperty",
		...
		if (!idAlreadyDefinded) {
			...
			fields.push({
				...
				kind: FFun({
					...
					params: [],
					args: [],
					expr: mk(EReturn(mk(EConst(CInt(Std.string(idCount))))))
				}),
				pos: pos
			});
			...
			for (i in array) {
				...
				if (i.meta.length != 0 && i.meta[0].name == "expose") {
					for (p in i.meta[0].params) {
						...
						switch (c) {
							case CString(s):
								code = "encode+= \"" + i.name + "." + s + ",f,\" +" + i.name + "." + s + "+\"?\""; // TODO add other types other than float
							default:
								{trace("meta data error " + c); continue;}
						}
						exp.push(Context.parseInlineString(code, Context.currentPos()));
					}
				} else {
			...
			for (i in array) {
				...
				if (i.meta.length != 0 && i.meta[0].name == "expose") {
					for (p in i.meta[0].params) {
						...
						switch (c) {
							case CString(s):
								code = "if (" + counter + "== id){" + i.name + "." + s +
									"=  Std.parseFloat(value); return;}"; // TODO add other types other than float
							default:
								{trace("meta data error " + c); continue;}
						}
						exp.push(Context.parseInlineString(code, Context.currentPos()));
		...
		fields.push({
			...
			kind: FFun({
				...
				params: [],
				args: [],
				expr: mk(EReturn(mk(EConst(CString(Md5.encode(md5String))))))
			}),
			pos: pos
		});
			...
			for (i in array) {
				...
				if (i.meta.length != 0 && i.meta[0].name == "expose") {
					for (p in i.meta[0].params) {
						...
						switch (c) {
							case CString(s):
								code = "if (" + counter + "== id){ return Std.string(" + i.name + "." + s + ");}"; // TODO add other types other than float
							default:
								{trace("meta data error " + c); continue;}
						}
						exp.push(Context.parseInlineString(code, Context.currentPos()));
